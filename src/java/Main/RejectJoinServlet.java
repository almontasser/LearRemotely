 ...
 * @author Mahmoud
 */
@WebServlet(name = "RejectJoinServlet", urlPatterns = {"/reject-join"})
public class RejectJoinServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        if ((Auth.isTeacher(req, resp) && subject.getTeacherId().equals(req.getSession().getAttribute("auth.id"))) || Auth.isAdmin(req, resp)) {
            DataAccess.executeUpdate("DELETE FROM subjects_students WHERE id="+id);
